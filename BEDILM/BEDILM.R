...
# Define physcial properties of the lake
    Zmax  <- 19  # maximum depth of profile (m): determines N2 for deepest layer
...
#========================#
  ...
  for (k in k.list)  {
      ...
      for (i in e.measurements) 
          ...
          #plot(log(e+1)~meas.depths)
          
          light.df$max.e[i] <- max(e, na.rm=T)
            
          light.model <- lm(log(e+1)~meas.depths)
          light.df$KD[i] <- as.numeric(coefficients(light.model)[2])
          
          options(warn = -1)
          light.df$e.r2[i] <- cor(log(e+1), light.model$fitted.values)**2 
          options(warn = 0)
          }
      ...
      # surface and Zmax temps for N2 calcs
      ...
      #--------------------------------------------------------
          ...
          for (i in 1:num.measurements) {
              ...
              p.Zmax <- (1-((6.63e-06)*((temp.Zmax-4)^2)))*1000  # water density (kg m^-3)
              ...
              N2[i,num.layers] <-  - ( (g/p.matrix[i,num.layers]) * ((p.matrix[i,num.layers] - p.Zmax)/(layer.thickness[num.layers])) )
          ...
          Kv[ Kv=="NaN" | Kv=="Inf"] <- 0 # when there is no density gradient (or negative density gradient) between layers
          Kv[Kv>1]<-1 # limit     
